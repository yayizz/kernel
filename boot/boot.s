    /* Decleare constants for the multiboot header. */
    ...
    /* allocates room for a small stack by creating a symbol at the bottom
    /* of it, then allocating 16384 bytes for it, and finally creating a
    /* symbol at the top. The stack grows downwards on x86. The stack is in its
    /* own section so it can be marked nobits, which means the kernel file is
    /* smaller because it does not contain an uninitialized stack. The stack
    /* on x86 must be 16-byte aligned according tı the stack is properly aligned
...
stack_top:
    ...
    /* been loaded. It doesn't make sense to return from this function
...
_start:
    ...
    /*  There is no printf function, unless the kernel provides its own
    /*  There are no security restrictions, no safeguard, no debugging mechanism
    /*  only what the kernel provides itself. It has absolute and complete ower
    /*  the machine */

    /* to set up a stack, we set the esp register to point to the top of the
    /* stack (as it grows downward on x86 systems). This is necessarily done
    /* in assembly as languages such as C cannot funnction without a stack */
    mov $stack_top, %esp
	...
	loaded here. Paging should be enabled here. */

    /*
	Enter the high-llevel kernel. The ABI requires the stack is 16-byte
	aligned at the time of the call instruction (which afterwards pushes
	the return pointer of size 4 bytes). The stack was originally 16-byte
	aligned above and we've pushed a multiple of 16 bytes tı the stack since
	(pushed 0 bytes so far), so the alignment has thus been preserved and the
	call is well defined. */
    call kernel_main

    /*
	If the system has nothing more to do, put the computer into an infinte
	loop. To do that:
	1) Disable interrupts with cli (clear interrupt enable in eflags). They
       are already disabled by the bootloader, so this is not needed.
	   Mind that you might later enable interrupts and return from kernel_main
	2) Wait for the next interrupt to arrive with hlt (halt instruction)
	   Since they are disabled, this will lock up the computer.
	3) Jumpy to the hlt instruction if it ever wakes up due to a non-maskable
